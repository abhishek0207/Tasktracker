...
$(".edit-btn").click(function (e) {
  ...
  var record_id = editbtn.data("record-id");
  var start_time = editbtn.data("start-time");
  var end_time = editbtn.data("end-time");
...
$("#edit_form_button").click(function (e) {
  var start_year = $("#editTimeblock_start_time_year").val();
  var start_month = $("#editTimeblock_start_time_month").val();
  var start_day = $("#editTimeblock_start_time_day").val();
  var start_hour = $("#editTimeblock_start_time_hour").val();
  var start_minute = $("#editTimeblock_start_time_minute").val();
  var end_year = $("#editTimeblock_end_time_year").val();
  var end_month = $("#editTimeblock_end_time_month").val();
  var end_day = $("#editTimeblock_end_time_day").val();
  var end_hour = $("#editTimeblock_end_time_hour").val();
  var end_minute = $("#editTimeblock_end_time_minute").val();
  var edit_start_time = new Date(start_year, start_month, start_day, start_hour, start_minute);
  //let edit_start_time = start_year + "-" + start_month + "-" + start_day + " " + start_hour + ":" + start_minute + ":"+ "00" + "." + "000000"
  var edit_end_time = new Date(end_year, end_month, end_day, end_hour, end_minute);
  //let edit_end_time = end_year + "-" + end_month + "-" + end_day + " " + end_hour + ":" + end_minute  + ":" + "00" + "." + "000000"
  var edit_task_id = $("#editTaskid").val();
  var edit_user_id = $("#edituserid").val();
  var edit_record_id = $("#editRecordid").val();
  console.log(edit_record_id);
  var jsonStart = JSON.stringify({
    timeblock: {
  ...
  $.ajax(timeBlockPath + "/" + edit_record_id, {
    ...
    success: function success() {
      alert('Timeblock Updated') ? "" : location.reload();
    }
  });
});
...
$(".delete-timeblock-btn").click(function (e) {
  ...
  var record_id = deletebtn.data("record-id");
  $.ajax(timeBlockPath + "/" + record_id, {
    ...
    data: "",
    success: function success() {
      alert('Record Delted successfully') ? "" : location.reload();
    }
  });
});
...
function addStartTimer(e) {
  ...
  var task_id = startBtn.data("task-id");
  var user_id = startBtn.data("user-id");
  var cur_timer = startBtn.data("time");
  var record_id = startBtn.data("record-id");
  if (record_id != "") {
...
function enterStartTime(curTask, curUser, curTimer, end_time) {
  ...
  $.ajax(timeBlockPath, {
    ...
    success: function success(resp) {
      set_button(curUser, curTask, resp.data.id);
    }
  });
}
...
function enterupdateTime(record_id, curTimer, curUser, curTask) {
  ...
  $.ajax(timeBlockPath + "/" + record_id, {
    ...
    data: jsonStart,
    success: function success(resp) {
      set_button(curUser, curTask, "");
    }
  });
}

function set_button(user_id, task_id, value) {
  $('.start-button').each(function (_, bb) {
    console.log(user_id);
    console.log(task_id);
...
function update_buttons() {
  $('.start-button').each(function (_, bb) {
    var user_id = $(bb).data('user-id');
    var task_id = $(bb).data('task-id');
    var record_id = $(bb).data('record-id');
    if (record_id == "") {
